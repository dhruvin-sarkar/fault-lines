...
<p><a href="https://dhruvin-sarkar.github.io/fault-lines/#collapse"><img src="assets/readme/collapse-race.gif" width="880" alt="The removal race on the live site: the flow-capacity curves of all six removal orders, each named at the playhead with the share it has left, above a scrubber and six maps of the central nervous system on a black field, one per removal order, each drawing every cell type as a square. As the scrubber steps from the intact graph to half of all types removed, the curves fall, red dots mark where each order halves flow capacity, removed types disappear and types cut off from sensory input turn grey, fastest under the targeted orders; under sensory-motor betweenness most of the remaining types turn grey in a single step. At the end the weighted out-degree map is selected, dimming the other orders and leaving its curve, which halves flow capacity at 4.1% removed."></a><br><sub>The race between the six removal orders, from the intact graph to half of all types removed, then the fastest, weighted out-degree, picked out.</sub></p>
...
<p><a href="https://dhruvin-sarkar.github.io/fault-lines/#lookup/ALIN7"><img src="assets/readme/lookup-alin7.gif" width="880" alt="The lookup section of the live site: typing ALIN7 into the search box filters the list of cell types, and choosing it opens its profile beside a map of where it sits in the nervous system. Scrolling down the profile shows its partner and synapse counts, what removing it alone breaks, which is 9 of 10,647 routes and no sensory-motor pair, and when each removal order removes it or cuts it off."></a><br><sub>Looking up the hidden bottleneck ALIN7.</sub></p>
...
<p><a href="https://dhruvin-sarkar.github.io/fault-lines/"><img src="assets/readme/site-phone-strip.png" width="880" alt="Four screens of the live site at phone width. The opening carries the title and the question over a map of the nervous system, one point per cell type, with a slider reading 4.9% of types removed and 44.0% of flow capacity left. The findings screen leads with the result that losing a few percent of cell types halves the routing, and the first key result, 4.1%, below it. The null-model screen sets the real graph against its randomizations: histograms of the 200 degree-preserving randomized graphs under random and weighted out-degree removal, each with a line at the real graph's score far outside the randomized range and marked significant. The last screen is the profile of the cell type ALIN7, with its counts and a table of what removing it alone breaks, 9 of 10,647 routes and no sensory-motor pair."></a><br><sub>The opening, the key results, the null model and a cell type profile at phone width.</sub></p>
