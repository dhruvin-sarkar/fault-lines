...
For each of the 11281 types with both a left and a right node, flow capacity was measured after removing the left node, the right node, and both. Impact = intact flow minus flow after removal. Superadditivity = impact(both) − impact(left) − impact(right). 4449 types have a non-zero impact in at least one of the three removals and enter the tests.

## Results

1. Removing both sides versus the mean of removing one side: W = 9899025 over 4449 types with unequal values, one-sided p = 0 (significant at 0.05).
2. Removing both sides versus the sum of the two single-side removals (superadditivity): W = 22236 over 214 types with unequal values, one-sided p = 5.2e-37 (significant at 0.05).
