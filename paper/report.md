...
The complete connectome of the adult male *Drosophila* central nervous system (neuPrint `male-cns:v1.0`) was represented as a directed graph of 11,751 cell types joined by 243,439 connections, each supplying at least 1% of its target's input. Sensory-to-motor routing was measured as the maximum number of edge-disjoint paths from 368 primary sensory types to 665 descending and motor types (10,647 in the intact graph) and as the number of connected sensory-motor pairs. Six adaptive removal strategies were applied under a pre-registered protocol. Removing cell types in order of weighted out-degree halves flow capacity after 4.1% of types are gone; random removal requires 28.3% (95% CI 27.7% to 28.9%), and every targeted strategy falls below that interval. Flow halves long before types become disconnected: at the half-flow point, at most 37 types have lost every path from sensory input. Removal by sensory-motor betweenness ends in an abrupt structural cascade in which a single batch disconnects 6,046 types, and at 50% removal 5,306 of the 5,846 remaining types have no sensory path, against 1 under random removal. Removing the strongest connections first halves flow after 26.8% of connections; removing the weakest half never does. Cascade sizes are not robustly power-law distributed (bootstrap p = 0.202 and 0.036 in two pooling schemes). Removing both hemispheric copies of a type is superadditive for 205 of 4,449 informative types and subadditive for 9 (one-sided Wilcoxon p = 5.2 × 10^−37^), while the other 4,235 are exactly additive. Fourteen behaviorally validated cell types rank above other types in sensory-motor betweenness (one-sided Mann-Whitney p = 0.045). All results describe a static, type-aggregated wiring diagram.
...
Both pre-registered tests are significant. Removing both sides costs more flow than the mean of the two single-side removals (W = 9,899,025 over 4,449 types with unequal values; the one-sided p underflows to zero in double precision). Removing both sides also costs more than the sum of the two single-side removals (W = 22,235.5 over 214 types with unequal values, one-sided p = 5.2 × 10^−37^). The effect is rare and small, however. Of the 4,449 informative types, 205 are superadditive, 4,235 additive and 9 subadditive, and the largest superadditivity is +6 paths (SApp10, whose left and right removals cost 179 and 162 paths and whose joint removal costs 347). Twelve types lose no flow when either side is removed alone but do lose flow when both are removed, so for these the contralateral homolog is complete structural insurance. Fourteen of the fifteen most superadditive types are sensory types; the exception is the ascending type ANXXX296 (Table 12).
...
| with MN9 | single-removal flow loss | 15 | 69,910 | 0.95 | 0.397 | 32.3 |
...
Two features of the sensitivity analyses deserve comment. Substituting DNp71 strengthens the primary result (p = 0.018). Including the positive control weakens it (p = 0.10), because MN9 scores zero on sensory-motor betweenness: it lies on no shortest sensory-to-motor route between other types, and removing it alone costs a single path of flow capacity. Its published importance lies in its motor output rather than in routing between the sensory and motor sets, so it is not a positive control for this score. The pre-registered plan excluded it from the primary test before any score was computed, and the primary result depends on that exclusion. The result is modest and rests on a small, non-random sample of well-studied neurons, many of which were found because they are large, genetically accessible or have striking phenotypes. It shows agreement between a structural ranking and behavioral experiments for these neurons, not that the ranking identifies behaviorally essential neurons in general.
...
A high structural criticality score is a hypothesis about where behaviorally relevant routing may concentrate, to be tested experimentally, not evidence that an animal depends on those neurons. The agreement with behaviorally validated cell types is modest and rests on 14 well-studied neurons. The comparison with published thresholds is qualitative, because breakdown criteria, graph resolution, edge direction and recalculation rules all differ between studies.
